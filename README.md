...
### Workflows: save a prompt, run it by name

A long instruction you repeat every day can become two words. Save it once:

> *"Create a workflow named standup making read my calendar and summarise my
> inbox."*

— and from then on, *"run workflow standup"* replays the exact prompt through
the normal pipeline: tools fire, panels appear, he speaks the answer. Also
supported: *"list my workflows"*, *"delete workflow standup"*, and a spoken
name with several words (*"run the workflow morning briefing"*). Workflows are
stored per-browser in `localStorage` — personal conveniences, not synced data.
...
| **M** | Mute the microphone (M again to unmute) |
