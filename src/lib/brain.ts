...
export function watchOsLog(fn: (text: string) => void): void {
  if (usingBridge) bridge.watchOsLog(fn)
}
export function watchSandboxImage(fn: (dataUrl: string) => void): void {
  if (usingBridge) bridge.watchSandboxImage(fn)
}
