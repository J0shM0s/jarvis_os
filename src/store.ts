...
/** Brain quota + the currently serving provider, as the brain proxy reports it. */
export type Quota = {
  limit: number | null
...
type State = {
  ...
  gestures: boolean
  /** True while M has muted the microphone: nothing is captured or
   *  transcribed, and the HUD says so. Typing in the composer still works —
   *  mute takes the ears away, not the whole assistant. */
  micMuted: boolean
  ...
  /** Business partner mode — separate brain, tone, and memory. Toggled with B. */
  businessMode: boolean
  /** Live OS log — jede Aktion vor Ausführung */
  osLogs: Array<{ id: string; text: string; at: number }>
  ...
  setBusinessMode: (on: boolean) => void
  toggleBusinessMode: () => void
  pushOsLog: (text: string) => void
  clearOsLogs: () => void
