...
import { memoryServer, getMemorySnapshot } from './memory.mjs'
...
function decideTool(name) {
  ...
  if (server) {
    ...
    // Memory — always allowed, it's the brain.
    if (server === 'jarvis_memory') return true
    // OS control — always allowed (GUI log is read, actions gated by ALLOW_WRITES via veto)
...
Business Partner Mode:
 ...
 - In business mode you still use all tools, but you think in frameworks: revenue, distribution, moat, next 3 actions. You push back when an idea is weak, you double down when it is strong.
 - You have a SEPARATE memory (category business). Use it. Business memory never leaks into private smalltalk unless asked.
 - Visual: the interface tints amber when this mode is active — you don't need to call ui_theme, the frontend does it.
 - Exit business mode only when the user toggles it again (they press B). Stay in character the whole session.

OS-Steuerung — Du hast volle Kontrolle über das OS + jede Desktop-App:
 - Tools: capture_screen, move_and_click, type_text, press_key, upload_file, launch_app, list_apps, sandbox_mode. Nutze sie.
 - Jede App geht: launch_app(app="WhatsApp"|"Rechner"|"Spotify"|"C:/Pfad/app.exe") öffnet direkt via Startmenü-Suche (Win → tippen → Enter) oder Pfad — im Sandbox isoliert, echter Desktop bleibt frei. list_apps zeigt alle 80 installierten Apps.
 - WORKFLOW: Immer zuerst capture_screen (mit hint), dann analysiere das Bild + Anweisung visuell, berechne Koordinaten, dann move_and_click/type_text/press_key. Für App-Start nutze launch_app statt manuellem Tippen.
 - Wenn du Aktionen ausführst, beschreibe deinen nächsten Schritt kurz für das GUI-Log (das Tool macht das automatisch via "JARVIS Log:"), und gib die OS-Befehle/Koordinaten aus.
...
const wss = new WebSocketServer({
  ...
  verifyClient: ({ origin, req }, done) => {
    console.log(`[jarvis] ws handshake origin=${origin ?? '(none)'} url=${req.url ?? '/'}`);
    ...
    console.log(`[jarvis] ws allowed origin=${origin ?? '(none)'}`);
    done(true)
  },
})
server.listen(PORT)
...
wss.on('connection', (socket) => {
  ...
  const session = query({
    ...
    options: {
      ...
      mcpServers: {
        ...
        // Long-term memory (general + business)
        jarvis_memory: memoryServer(),
        // OS-Steuerung (pyautogui + Pillow) — Maus/Tastatur/Screen, non-blocking mit Live-Log + Sandbox-Live-Bild
        jarvis_os: osServer(
          (msg) => send({ type: 'os_log', log: msg }),
          (dataUrl) => send({ type: 'sandbox_image', data: dataUrl })
        ),
        ...
        switch (msg.type) {
          ...
          case 'result':
            ...
            if (msg.subtype === 'success') {
               ...
               */
              if (result.length < 200 && /not logged in|please run \/login/i.test(result)) {
  ...
  socket.on('message', (raw) => {
    ...
    if (msg.type === 'ask' && typeof msg.text === 'string') {
       ...
       */
      let text = msg.text
      // Business mode: prefix so the persona switches without a new session.
      if (msg.businessMode) {
        text = `[BUSINESS MODE ACTIVE — you are the business partner, not the butler. Use business memory category.]\n${text}`
      }
